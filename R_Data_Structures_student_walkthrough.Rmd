...
#IN R INDEXES START AT 1 AND NOT 0
names[1] 
```
...
```{r}
vector_1 <- c(1, 2, 3)
vector_2 <- c(4, 5, 6, 7, 8, 9) #seven is added to one, etc.
...
```{r}
student_grades <-  c(91, 94, 95, 75, 82)
...
student_list #similar to data frame or list of lists
#2d matrix
```
...
student_grade_level <- c("Freshman", "Junior")
student_class_grade <- c(98, 81)
...
my_matrix #note: goes down then up again
```
...
my_matrix <-  matrix(
  ...
  ncol <- 5,
  byrow <- FALSE,
  dimnames <- list(c("Row 1","Row 2", "Row 3"), c("Col 1", "Col 2","Col 3","Col 4","Col 5"))
)
my_matrix  
```
...
my_factor <- factor(c("Blue","Green","Brown","Blue","Blue","Green","Brown","Gray"))
my_factor
```
...
```{r}
my_factor <- factor(
  c("Blue","Green","Brown","Blue","Blue","Green","Brown","Gray"),
  levels <- c("Blue","Green","Brown","Gray")
)
my_factor
...
big_array <- array(
  ...
  dim <- c(3,3,2),
  # row/column names
  dimnames <- list(c("row uno","row dos", "row tres"),c("col uno","col dos", "col tres"), c("matrix uno","matrix dos")
))
big_array
```
